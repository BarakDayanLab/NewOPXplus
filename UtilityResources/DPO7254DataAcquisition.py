...
class DPO7254Visa():
    def __init__(self, ip='169.254.231.198'):
        ...
        self.dataString = ''
        self.timeData = None # placeholder
        self.wvfm = [[]] * 5  # assuming channels 1-4
        if 'TEKTRONIX,DPO7254' not in self.ID and 'TEKTRONIX,DPO70604' not in self.ID:
            printError('Could not identify instrument. Check connection, try again.')
            print(self.ID)
            return
        printGreen(self.ID)
        self.scopeType = 'DPO7254' if 'TEKTRONIX,DPO7254' in self.ID else 'DPO70604'
    ...
    def acquireData(self, chns=(1, 2, 3, 4)):
        for ch in chns:
            ...
            #self.write('DATA:STOP {}'.format(nDataPoints))  # make sure we bring back entire data
            # Acquisition
    ...
    def saveData(self, filename=''):
        ...
        fPath = os.path.join(todaydatadir, nowformated)
        meta = "Traces from the DPO7254 scope, obtained on %s at %s.\n" % (todayformated, nowformated)
        np.savez_compressed(fPath + '.txt', Ch_1_Data=self.wvfm[1],
    ...
    def savePlotsAndData(self, filename='', saveFig = False):
        if filename != '':
            filename = ' - ' + str(filename)
        ...
        if saveFig:
            ...
            ax1.plot(self.timeData, self.wvfm[1], color='g')
            # ax1.set_ylabel('Temperature [K]', color='b')
            ax1.set_xlabel('Time')
            ax2.plot(self.timeData, self.wvfm[2], color='b')
            # ax2.set_ylabel('Detuning [GHz], of F2->F\'3', color='r')
            # ax2.invert_yaxis()
            plt.grid()


            figPath = dataFilePath + '.png' #os.path.join(todaydatadir, nowformated + ".png")
