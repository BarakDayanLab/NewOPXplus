...
def QRAM_Exp_Gaussian_samples(sprint_pulse_len=110, det_pulse_len=30, det_pulses_amp=[0.4]*6, sprint_pulses_amp=[0.4]*4,
    ...
    qram_exp_gaussian_samples = [0] * (num_init_zeros + 9)
    ...
    qram_exp_gaussian_samples += (signal.gaussian((sprint_pulse_len-2), std=((sprint_pulse_len-2) * 0.5 / 2.355)) * det_pulses_amp[-1]).tolist() + [0] * (num_between_zeros)  # -3 for echos from south
    ...
    qram_exp_gaussian_samples = qram_exp_gaussian_samples[:-(num_between_zeros)] + [0] * (num_between_zeros + num_fin_zeros)
...
def QRAM_Exp_Square_samples(amp=0.45, sprint_pulse_len=110, det_pulse_len=30, det_pulses_amp=[0.4]*6, sprint_pulses_amp=[0.4]*4,
    ...
    qram_exp_gaussian_samples = [det_pulses_amp[0] * amp] * (num_init_zeros + 9)
    ...
    qram_exp_gaussian_samples += [det_pulses_amp[-1] * amp] * (sprint_pulse_len - 2) + \
                                 [det_pulses_amp[-1] * sprint_pulses_amp[0] * amp] * (num_between_vals)

    qram_exp_gaussian_samples += [sprint_pulses_amp[0] * amp] * (num_mid_val - 2)  # due to unresolved reflections

    for m in range(len(sprint_pulses_amp)-1):
        qram_exp_gaussian_samples += [sprint_pulses_amp[m] * amp] * (sprint_pulse_len) + \
                                     [sprint_pulses_amp[m] * sprint_pulses_amp[m + 1] * amp] * (num_between_vals)
    qram_exp_gaussian_samples += [sprint_pulses_amp[-1] * amp] * (sprint_pulse_len) + \
...
# QRAM_Exp_Gaussian_samples_S =[0.45]*516
#
# QRAM_Exp_Gaussian_samples_N=[0.45]*516
...
# QRAM_Exp_Gaussian_samples_S = QRAM_Exp_Square_samples(amp=Pulses_Amp,
#                                                       sprint_pulse_len=sprint_pulse_len,
#                                                       det_pulse_len=det_pulse_len,
#                                                       det_pulses_amp=det_pulse_amp_S,
#                                                       sprint_pulses_amp=sprint_pulse_amp_S,
...
QRAM_MZ_balance_pulse_North = ([0] * AOM_risetime + [Pulses_Amp_balance] * (MZ_delay - AOM_risetime - AOM_risetime_pulsers) + [0] * AOM_risetime_pulsers
...
#                               MZ_balancing_seq_rep
QRAM_MZ_balance_pulse_South = ([0] * AOM_risetime + [0] * (MZ_delay - AOM_risetime - AOM_risetime_pulsers) + [0] * AOM_risetime_pulsers
...
# QRAM_Exp_Gaussian_samples_S = ([0] * AOM_risetime + [0] * (MZ_delay - AOM_risetime - AOM_risetime_pulsers) + [0] * AOM_risetime_pulsers
#                                + [0] * AOM_risetime + [0] * (MZ_delay - AOM_risetime - AOM_risetime_pulsers) + [0] * AOM_risetime_pulsers) * \
#                               2
...
# QRAM_Exp_Gaussian_samples_N = ([Pulses_Amp_balance] * AOM_risetime + [Pulses_Amp_balance] * (MZ_delay - AOM_risetime - AOM_risetime_pulsers) + [Pulses_Amp_balance] * AOM_risetime_pulsers
...
#                                + [0] * (MZ_delay - AOM_risetime_pulsers) + [Pulses_Amp_balance] * (AOM_risetime_pulsers - 20) + [0] * 20) * 2
# QRAM_Exp_Square_samples_Late = ([0] * MZ_delay + [Pulses_Amp_Late] * MZ_delay) * 2
