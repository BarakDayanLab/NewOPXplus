...
all_elements = ["Cooling_Sequence", "MOT_AOM_0", "MOT_AOM_-", "MOT_AOM_+", "AntiHelmholtz_Coils",
                "FLR_detection", "Measurement", "AOM_2-2/3'", "AOM_2-2/3'_detuned", "AOM_2-3'_for_interference"]
...
def MOT(mot_repetitions):
    ...
    align("Cooling_Sequence", "MOT_AOM_0", "MOT_AOM_-", "MOT_AOM_+", "AntiHelmholtz_Coils", "Zeeman_Coils",
          "AOM_2-2/3'", "AOM_2-2/3'_detuned", "AOM_2-3'_for_interference", "FLR_detection", "Measurement")#, "Dig_detectors",
...
def FreeFall(freefall_duration, coils_timing):
    ...
    align("Cooling_Sequence", "MOT_AOM_0", "MOT_AOM_-", "MOT_AOM_+", "Zeeman_Coils", "AOM_2-2/3'",
          "AOM_2-2/3'_detuned", "AOM_2-3'_for_interference", "Measurement") #, "Dig_detectors", "Dig_detectors_spectrum")  # , "AOM_N", "AOM_S")
...
#
#
# def Probe_counts_Measure_SNSPDs(M_delay, m_time, m_window, shutter_open_time,
#                                 ON_counts_st1, ON_counts_st2, ON_counts_st3, ON_counts_st4,
#                                 ON_counts_st5, ON_counts_st6, ON_counts_st7, ON_counts_st8, ):
#     """
...
#
#
# def Spectrum_Experiment(M_delay, m_time, m_window, shutter_open_time,
#                         ON_counts_st5, ON_counts_st6, ON_counts_st7, ON_counts_st8,
#                         tt_st_S, rep_st):
#     """
#        Transit measurement with and without atoms.
...
#     with for_(t, 0, t < (m_time * 4), t + (2 * len(Config.Spectrum_Exp_Gaussian_samples))):
#         play("OD_Gaussian_Pulse", "AOM_2-2/3'_detuned")
#         wait(int(len(Config.Spectrum_Exp_Gaussian_samples) / 4), "AOM_2-2/3'", "AOM_2-2/3'_detuned")
...
#         with for_(m8, 0, m8 < counts8, m8 + 1):
#             save(tt_vec8[m8], tt_st_S)
#         with for_(j, 0, j < (8000 - counts5 - counts6 - counts7 - counts8), j + 1):
#             save(zero, tt_st_S)
#         with for_(m, 0, m < 8000, m + 1):
#             save(n, rep_st)
...
def opx_control(obj, qm):
    with program() as opx_control_prog:
        ...
        with infinite_loop_():
            ...
            with for_(k, 1, k <= N_Snaps + Buffer_Cycles, k + 1):
                with for_(j, 1, j <= filler, j + 1):
                    ...
                    # with else_():
                    wait(PrePulse_duration - shutter_open_time, "Cooling_Sequence", "Measurement")
                    align(*all_elements,  "AOM_2-2/3'")
                    ...
                    with if_((Imaging_Phase == 4) & (Pulse_1_duration > 0)):  # 4 means imaging phase on pulse_1
                        ...
                        with else_():
                            ...
                            with if_(OD_FS_pulse_duration > 0):
                                ...
                                align(*all_elements, "AOM_2-2/3'")
                            with else_():
                                # with if_(Spectrum_Exp_ON):
                                #     align(*all_elements, "Dig_detectors_spectrum", "AOM_2-2/3'",
                                #           "AOM_2-2/3'_detuned")
                                #     Spectrum_Experiment(PrePulse_duration, M_duration, obj.M_window, shutter_open_time,
                                #                         ON_counts_st5, ON_counts_st6, ON_counts_st7, ON_counts_st8,
                                #                         tt_st_S, rep_st)
                                ...
                                #         Probe_counts_Measure_SNSPDs(PrePulse_duration, M_duration, obj.M_window,
                                #                                     shutter_open_time, ON_counts_st1, ON_counts_st2,
                                #                                     ON_counts_st3, ON_counts_st4, ON_counts_st5, ON_counts_st6,
        ...
        #     (tt_st_S + rep_st).buffer(8000 * obj.rep).save('South_Probe_TT')
...
class OPX:
    def __init__(self, config=Config.config):
        ...
        self.QuadRFControllers.append(QuadRFFrequencyScannerController(MOGdevice = qrfContr.dev, channel=2, debugging=True))  # updates values on QuadRF (uploads table)
    ...
    def Save_SNSPDs_Transit_Measurement_with_tt(self, N, histogram_bin_size, Transit_profile_bin_size, preComment,
                                                counts_threshold, intensity_threshold, max_probe_counts):
        """
        ...
        time_threshold = int(
            histogram_bin_size * 0.5)  # The minimum time between two time tags to be counted for a transit. # TODO: might need a factor of 2???
        ...
        while True:
            ...
            if ((len(tt_S_measure) * 1000) / self.M_time) < counts_threshold:
                ...
                if len(self.tt_S_measure_batch) == N:
                    tt_N_transit_events[
                        [i for i, x in enumerate(tt_N_binning_batch[0]) if x > intensity_threshold]] -= 1
                    tt_S_transit_events[
                        [i for i, x in enumerate(tt_S_binning_batch[0]) if x > intensity_threshold]] -= 1
    ...
    def Save_SNSPDs_Spectrum_Measurement_with_tt(self, N, histogram_bin_size, Transit_profile_bin_size, preComment,
        ...
        histogram_bin_number = self.M_time // (histogram_bin_size * 2)
        time_bins = np.linspace(0, self.M_time, histogram_bin_number)
        ...
        # Place holders for results
        # while (number of photons * 10^-6 [Mcounts] / Measuring time [nsec] * 10^-9 [sec/nsec])  > counts_threshold [Mcounts /sec]:
        while (((sum(tt_S_binning_resonance) * 1000) / (self.M_time / 2)) > total_counts_threshold) or start:
            if self.keyPress == 'ESC':
                print('\033[94m' + 'ESC pressed. Stopping measurement.' + '\033[0m')  # print blue
            ...
            tt_S_binning = np.zeros(histogram_bin_number * 2)

            for x in self.tt_S_measure:
                tt_S_binning[x // histogram_bin_size] += 1

            # split the binning vector to odd and even - on and off resonance pulses
            tt_S_binning_resonance = [tt_S_binning[x] for x in range(len(tt_S_binning)) if x % 2]  # odd
            tt_S_binning_detuned = [tt_S_binning[x] for x in range(len(tt_S_binning)) if not x % 2]  # even
            ...
            if value:
                if ((index - current_transit[0]) * 480) < transit_time_threshold:
                    ...
                    for i in range(current_transit[0], current_transit[-1]):
                        ...
                        Cavity_atom_spectrum[((((i + 1) * len(Config.Spectrum_Exp_Gaussian_samples) * 2) %
                                               experiment.M_window) // (len(Config.Spectrum_Exp_Gaussian_samples) * 1000)) %
                                             spectrum_bin_number] += tt_S_binning_detuned[i + 1]
        ...
        for index, value in enumerate(tt_S_binning_detuned):
            if index not in [vec for elem in all_transits for vec in elem]:
                Cavity_spectrum[(((index * len(Config.Spectrum_Exp_Gaussian_samples) * 2) %
                                  experiment.M_window) // (len(Config.Spectrum_Exp_Gaussian_samples) * 1000)) %
                                spectrum_bin_number] += value
        ...
        while True:
            ...
            props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
            textstr_detuned = r'$Probe_N = %.3f$' % ((sum(tt_S_binning_detuned) * 1000) / (self.M_time / 2),) + '[MPhotons/sec]\n' \
                       + '$\overline{Probe}_S = %.3f$' % ((np.mean([sum(x) for x in tt_S_binning_detuned_batch]) * 1000)
                                                          / (self.M_time / 2),) + '[MPhotons/sec]'
            textstr_resonance = r'$Probe_S = %.3f$' % ((sum(tt_S_binning_resonance) * 1000) / (self.M_time / 2),) + '[MPhotons/sec]\n' \
            ...
            # ax1.plot([sum(time_bins[i:i+4]) for i in range(0, len(time_bins), 4)],
            ...
            # ax2.plot([sum(time_bins[i:i+4]) for i in range(0, len(time_bins), 4)],
            ...
            ax3.plot(freq_bins, Cavity_atom_spectrum, label='Cavity-Atom Spectrum', color='k')
            ax3.set(xlabel='Frequency [MHz]', ylabel='Counts [#Number]')
            ax3.legend(loc='upper right')

            ax4.plot(freq_bins, Cavity_spectrum, label='Cavity Spectrum', color='k')
            ...
            if ((sum(tt_S_binning_resonance) * 1000) / (self.M_time / 2)) < total_counts_threshold:
                    ...
                    if value:
                        if ((index - current_transit[0]) * 480) < transit_time_threshold:
                            ...
                            for i in range(current_transit[0], current_transit[-1]):
                                ...
                                Cavity_atom_spectrum[((((i + 1) * len(Config.Spectrum_Exp_Gaussian_samples) * 2) %
                                                       experiment.M_window) // (
                                                                  len(Config.Spectrum_Exp_Gaussian_samples) * 1000)) %
                                                     spectrum_bin_number] += tt_S_binning_detuned[i + 1]
                            current_transit = [index]
                        else:
                            # Finding if there any index that was saved to current transit and is close enough to the new index
        ...
        filename_S_tt = f'South_timetags.npz'
        filename_S_transits = f'South_Transits.npz'
        filename_Cavity_spectrum = f'Cavity_Spectrom.npz'
        filename_AtomCavity_spectrum = f'AtomCavity_Spectrom.npz'
        filename_FLR = f'Flouresence.npz'
        filename_timestamp = f'Drops_time_stamps.npz'
        ...
        if len(Exp_timestr_batch) > 0:
            np.savez(dirname + filename_timestamp, Exp_timestr_batch)
        if len(Cavity_spectrum) > 0:
            np.savez(dirname + filename_Cavity_spectrum, Cavity_spectrum)
        if len(Cavity_atom_spectrum) > 0:
            np.savez(dirname + filename_AtomCavity_spectrum, Cavity_atom_spectrum)
        if len(self.tt_S_measure_batch) > 0:
            np.savez(dirname_S + filename_S_tt, self.tt_S_measure_batch)
        if len(all_transits_batch) > 0:
            np.savez(dirname_S + filename_S_transits, all_transits_batch)
    ...
    def Start_Transit_Exp_with_tt(self, N=100, Histogram_bin_size=1000, Transit_profile_bin_size=100, preComment=None,
                                  threshold=1, intensity_threshold=5):
        ...
        self.Save_SNSPDs_Transit_Measurement_with_tt(N, Histogram_bin_size, Transit_profile_bin_size, preComment,
                                                     threshold, intensity_threshold, Max_probe_counts)

    def Start_Spectrum_Exp_with_tt(self, N=100, Histogram_bin_size=len(Config.Spectrum_Exp_Gaussian_samples),
                                   Transit_profile_bin_size=100, preComment=None, total_counts_threshold=1,
                                   transit_counts_threshold=5, transit_time_threshold=5000, bandwidth=80,
                                   freq_step=4):
        # Max_probe_counts = self.Get_Max_Probe_counts(3)  # return the average maximum probe counts of 3 cycles.
        Max_probe_counts = None  # return the average maximum probe counts of 3 cycles.
        self.Spectrum_Exp_switch(True)
        self.update_parameters()
        self.Save_SNSPDs_Spectrum_Measurement_with_tt(N, Histogram_bin_size, Transit_profile_bin_size, preComment,
